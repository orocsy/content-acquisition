...
function flattenModuleToc(moduleLike, coursePrefix, out = [], sectionPath = []) {
  const directTocEntries = Array.isArray(moduleLike?.toc) ? moduleLike.toc : null;
  if (directTocEntries) {
    for (const tocEntry of directTocEntries) {
      const tocSectionPath = tocEntry && tocEntry.title && tocEntry.title !== '__default'
        ? [...sectionPath, tocEntry.title]
        : [...sectionPath];
  ...
  const categories = Array.isArray(moduleLike) ? moduleLike : (moduleLike?.toc?.categories || moduleLike?.categories || []);
...
async function discoverCurriculumFromPalApi(page, coursePrefix) {
  const current = await page.evaluate(() => ({ path: location.pathname }));
  ...
  const collectionResult = await page.evaluate(async ({ courseSlugInner, pageSlugInner }) => {
  ...
  if (collectionResult.status !== 200 || !collectionResult.json) {
    throw new Error(`Collection API returned status ${collectionResult.status}`);
  }

  const pathAuthorId = Number(collectionResult.json?.path_author_id || collectionResult.json?.instance?.details?.path_author_id || 0);
  const pathId = Number(collectionResult.json?.path_id || collectionResult.json?.instance?.details?.path_id || 0);
  if (!pathAuthorId || !pathId) {
    throw new Error('Could not determine path_author_id/path_id from collection API');
  }

  const palResult = await page.evaluate(async ({ pathAuthorIdInner, pathIdInner }) => {
    const res = await fetch(`/api/pal/${pathAuthorIdInner}/${pathIdInner}?get_customizable_toc=false`, { credentials: 'include' });
    const json = await res.json();
    return { status: res.status, json };
  }, { pathAuthorIdInner: pathAuthorId, pathIdInner: pathId });

  if (palResult.status !== 200 || !palResult.json) {
    throw new Error(`PAL API returned status ${palResult.status}`);
  }

  const pathDetails = palResult.json?.instance?.details || {};
  const topLevelItems = pathDetails?.toc?.categories || [];
  const orderedLessons = [];
  const moduleTitles = [];

  for (const item of topLevelItems) {
    if (item?.type !== 'PATH_INTERNAL_MODULE') continue;
    if (item?.title) moduleTitles.push(item.title);
    flattenModuleToc(item, coursePrefix, orderedLessons, item?.title ? [item.title] : []);
  }

  return {
    source: 'pal-path-api',
    courseRootUrl: normalizeCourseRootFromPrefix(coursePrefix),
    title: pathDetails.title || courseSlug,
    headings: moduleTitles,
    authorId: pathAuthorId,
    pathId,
    orderedLessons,
    rawCategoryCount: topLevelItems.length,
    skippedNonLessonItems: topLevelItems.filter((x) => x?.type !== 'PATH_INTERNAL_MODULE').length,
  };
}
